...
const Notifications = require("../../models/Notification/NotificationSchema");
...
exports.getProducts = async (req, res) => {
    try {
        ...
        if (status && status === "stockOut") {
            andConditions.push({
                totalStock: { $eq: 0 },
            });
        }

        if (status && status === "stockLow") {
            andConditions.push({
                totalStock: { $ne: 0, $lte: 10 },
            });
        }
        ...
        const { minimumStock, outStock, totalNumberItems } =
            await getStocksInfo(allProducts);
...
exports.getAllProducts = async (req, res) => {
    try {
        ...
        // Calculate stock metrics
        const minimumStock = productsWithStock.filter(
            (p) => p.totalStock <= 10 && p.totalStock > 0
        ).length;
        const outStock = productsWithStock.filter(
            (p) => p.totalStock === 0
        ).length;
...
exports.getCustomProducts = async (req, res) => {
    try {
        const products = await Product.find(
            {},
            { _id: 1, productName: 1, unitSize: 1, unit: 1, sellingPrice: 1 }
        )
            .sort({ createdAt: -1 })
            .lean();
...
exports.addProduct = async (req, res) => {
    try {
        const { productName, sellingPrice, createdBy, category, ...remaining } =
            req.body;

        console.log(productName, sellingPrice, createdBy, category, remaining);
        ...
        if (!user) {
            console.log(`No user found for createdBy ID: ${createdBy}`);
            ...
            if (admins.length === 0) {
                ...
                const notifications = admins.map((admin) => ({
                    message: `Staff ${user.firstname} created a new product: ${product.productName}`,
                    type: "SYSTEM",
                ...
                console.log(
                    `Created ${inserted.length} notifications for product ${product.productName}`
                );
            }
        } else {
            console.log(
                `User ${user.firstname} is not a STAFF member, skipping notifications`
            );
        }
        ...
        if (req.file && req.image) {
            if (req.image && req.image.cloudinary_id) {
                await cloudinary.uploader.destroy(req.image?.cloudinary_id);
            }
        }
...
exports.updateProduct = async (req, res) => {
    try {
        ...
        const { minimumStock, outStock, totalNumberItems } =
            await getStocksInfo(allData);

        const user = await Users.findById(updatedBy);
        if (!user) {
            console.log(`No user found for updatedBy ID: ${updatedBy}`);
        } else if (user.role === "STAFF") {
            const admins = await Users.find({ role: "ADMIN" });
            if (admins.length === 0) {
                console.log("No admins found to notify");
                ...
                const notifications = admins.map((admin) => ({
                    message: `Staff ${user.firstname} Updated a new product: ${product.productName}`,
                    type: "SYSTEM",
                    relatedEntity: product._id,
                    entityType: "Product",
                    recipient: admin._id,
