...
exports.getTransactions = async (req, res) => {
  try {
    ...
    let startDate = null;
    let endDate = null;
    let transactionType = req.query.type || "SALE";
    let supplierCompanyName = "";

    if (req.query) {
      const { startOfDate, endOfDate, type } = req.query;
      startDate =
        startOfDate && !isNaN(new Date(startOfDate).getTime())
          ? new Date(startOfDate)
          : new Date();
      endDate =
        endOfDate && !isNaN(new Date(endOfDate).getTime())
          ? new Date(endOfDate)
          : new Date();

      startDate.setUTCHours(0, 0, 0, 0);
      endDate.setUTCHours(23, 59, 59, 999);
      transactionType = typeof type === "string" && type ? type.trim() : "SALE";
    }
    ...
    const pipeline = [
      ...
      },
      {
        $match: {
          ...
          ...(supplierCompanyName && {
            $or: [{ "supplier.companyName": escapedSupplierCompanyName }],
          }),
        },
      },
      {
  ...
  try {
    ...
    if (!transactionType || !items || !totalAmount || !createdBy) {
      return res
        ...
        .json({
          error:
            "Missing required fields: transactionType, items, totalAmount, createdBy",
        });
    }
    ...
    if (!["SALE", "DAMAGE"].includes(transactionType)) {
      return res
        .status(400)
        .json({ error: "Invalid transactionType. Must be SALE or DAMAGE" });
    }
    ...
    for (const item of items) {
      ...
      if (item.unitPrice < 0) {
        return res
          .status(400)
          .json({ error: "Unit price must be non-negative" });
      }
    }
    ...
    if (calculatedTotal !== totalAmount) {
      return res
        .status(400)
        .json({ error: "Total amount does not match calculated total" });
    }
...
exports.markNotificationAsRead = async (req, res) => {
  try {
    ...
    if (!notificationId || !userId) {
      return res
        .status(400)
        .json({ error: "Notification ID and User ID are required" });
    }

    const notification = await mongoose
      .model("Notification")
    ...
    if (!notification.recipients.includes(userId)) {
      return res
        .status(403)
        .json({
          error: "User not authorized to mark this notification as read",
        });
    }
    ...
    if (
      notification.recipients.every((recipient) =>
        notification.readBy.some(
          (read) => read.user.toString() === recipient.toString()
        )
      )
    ) {
