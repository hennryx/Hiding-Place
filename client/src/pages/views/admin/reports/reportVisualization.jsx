    ...
    useEffect(() => {
        ...
        switch (reportType) {
            case "SALE":
                ...
                data.forEach((item) => {
                    ...
                    const amount = item.items.reduce((sum, p) => {
                        return (
                            sum + p.quantity * (p.product?.sellingPrice || 0)
                        );
                    }, 0);
                ...
                processedData = Object.entries(salesByDate).map(
                    ([date, amount]) => ({
                        name: date,
                        value: amount,
                        sales: amount,
            ...
            case "PURCHASE":
                ...
                processedData = Object.entries(purchasesBySupplier).map(
                    ([supplier, count]) => ({
                        name: supplier,
                        value: count,
                        items: count,
            ...
            case "INVENTORY":
                ...
                summary.total = sortedProducts.reduce(
            ...
            case "RETURN":
                ...
                data.forEach((item) => {
                    ...
                    const returnCount = item.items.reduce(
                        (sum, p) => Number(sum) + Number(p.quantity),
                        0
                    );
                ...
                processedData = Object.entries(returnsByReason).map(
                    ([reason, count]) => ({
                        name:
                            reason.length > 20
            ...
            <div className="flex flex-wrap justify-around mb-4 bg-gray-50 rounded-md p-4">
                <div className="text-center p-2">
                    ...
                    <p className="text-xl font-semibold">
                        {reportType === "sales" ? "₱" : ""}
                        {summaryData.total.toFixed(
                            reportType === "sales" ? 2 : 0
                        )}
                    </p>
                </div>
                <div className="text-center p-2">
